...
from pyloggi.exceptions import InvalidFileType
...
@pytest.mark.parametrize(("console_mode", "file_mode"), [("mark", True), (True, "admin")])
...
def test_unknown_color(color : str):
    with pytest.raises(ValidationError):
        ColorConfig(enabled_console_color=True,debug=color,info=color,warning=color,error=color,critical=color)
